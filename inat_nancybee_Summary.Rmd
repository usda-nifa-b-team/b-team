...
params: 
  collectorName: ""    
title: "`r params$collectorName` Summary Sheet"
...
# volDat <- l3joined %>% filter(collector %in% params$collectorName) 
#   
# singles <- filter(singles, collector %in% params$collectorName)
...
```{r volMap, warning=FALSE, fig.cap= paste0("Bee catch locations for ", collString,", along with total catches per county.")}
...
p1 <- dat %>% st_drop_geometry() %>%  count(collector) %>% 
  ...
  mutate(isCol=collector=="inat nancybee") %>% 
