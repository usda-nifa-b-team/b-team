...
#Overwrite county name using shapefiles, using lat/lon data from individual records (takes ~10 sec) - a fair bit longer in 2025!
countyMatch <- sapply(st_within(dat,orCounties),function(x) x) #Get county indices for locations

if(any(!sapply(countyMatch,length)!=0)){
  warning(paste0(sum(!sapply(countyMatch,length)!=0),' samples located outside of Oregon discarded'))
...
if(any(sum(dat$county!=orCounties$altName[unlist(countyMatch)]))){
  warning(paste0(sum(dat$county!=orCounties$altName[unlist(countyMatch)]),' recorded county names are not inside matching county borders. Changed to match counties.'))
  dat$county <- orCounties$altName[unlist(countyMatch)]
}
...
save.image('Robinson/Data/cleaned2025.Rdata') #Cleaned data + helper function
