...
template <- readLines('templateSheet.Rmd') #Rmarkdown template to use
...
setwd('./Output_Sheets') #Move to Output_Sheets directory
...
for(i in 1:nrow(volNames)){ 
  ...
  render(input=volNames$rmdFile[i],output_file=volNames$pdfFile[i],quiet=TRUE,clean=TRUE,envir=new.env())
