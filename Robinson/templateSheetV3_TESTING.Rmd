...
```{r volMap, warning=FALSE, fig.cap= paste0("Bee catch locations for ", collString, " (within ", stateNameNice, "), along with total catches per county.")}
...
```{r, eval= params$state %in% c("ID", "NM", "CO", "AZ"), child= "V3_child_phen_non.Rmd"}
```
...
In total, volunteers from the `r stateNameNice` Bee Atlas project identified `r round(100*with(accuracyAll,sum(genusID)/sum(nSpecimens)),1)` % (`r sum(accuracyAll$genusID)`) of the `r sum(accuracyAll$nSpecimens)` bee specimens to the level of genus, with an average accuracy of `r round(100*with(accuracyAll,sum(genusCorrect)/sum(genusID)),1)`%. Volunteers also identified `r round(100*with(accuracyAll,sum(genSppID)/sum(nSpecimens)),1)`% (`r sum(accuracyAll$genSppID)`) of the specimens to species level, and had an average accuracy of `r round(100*with(accuracyAll,sum(genSppCorrect)/sum(genSppID)),1)`% (see Table \@ref(tab:detAccuracyAll)). Nicely done!
