---
title: "Heidi Ashworth Summary Sheet"
...
#Volunteer data only
volDat <- dat %>% filter(collector=="Heidi Ashworth") 
  
singles <- filter(singles,collector=="Heidi Ashworth")
...
Heidi Ashworth caught `r nrow(volDat)` bees across `r length(unique(volDat$county))` `r ifelse(length(unique(volDat$county))>1,"counties","county")` from  `r format(min(volDat$date),format="%B %d, %Y")` to `r format(max(volDat$date),format="%B %d, %Y")`, representing `r length(unique(volDat$genSpp))` unique species. `r ifelse(nrow(singles)>0,paste0("Heidi Ashworth also caught the only _",singles$genSpp[1],"_ in the collection!"),"")`

```{r, out.width="100%", fig.cap = "Bees caught by Heidi Ashworth, broken down by species, genus, and family."}
abundPlots(volDat,colSet="Set1")
```

```{r volMap, warning=FALSE, fig.cap="Bee catch locations for Heidi Ashworth, along with total catches per county."}
...
p1 <- dat %>% st_drop_geometry() %>%  count(collector) %>% 
  ...
  mutate(isCol=collector=="Heidi Ashworth") %>% 
