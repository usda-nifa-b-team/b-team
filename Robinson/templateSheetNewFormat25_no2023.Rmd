...
singlesR <- singles %>% filter(str_detect(collector, collString))
...
accuracy <- dataNoNA %>% st_drop_geometry() %>% 
...
```{r, include= FALSE}
volDat <- dataNoNA %>% filter(str_detect(collector, collString))

volAccuracy23 <- accuracy23 %>% filter(str_detect(collector, collString))

volAccuracyAll <- accuracyAll %>% filter(str_detect(collector, collString))

volDatAllYrs <- data %>% filter(str_detect(collector, collString)) # for 2nd page of aggregate bees
```
...
textSizeA <- c(3.21*log10(NsppA)^(-3.17), 1.26*log10(NgenA)^(-2.12), 1)
...
abundPlots(volDatAllYrs,colourSet="Set1")
```
...
#These equations appear to make OK text size adjustments
textSize <- c(3.21*log10(Nspp)^(-3.38), 1.26*log10(Ngen)^(-2.12), 1)
...
```{r allCatches, fig.height=9,fig.width=8, fig.cap = paste0("Number of bee specimens and unique bee species caught by all volunteers, with your effort shown in red.")}
