...
```{r famID plot, fig.width=5, fig.height= 4, fig.cap= "Proportion of each bee family identified to species"}
...
A total of `r nSpp` bee taxa have been observed in `r params$state`. There have been a total of `r nObs` bees collected by `r nCol` individual collectors, from `r nPlntSpp` unique plant taxa. 
...
Note that some invasive weeds can support significant bee diversity and abundance. When picking plants to support bees, consider replacing exotic species with a functionally equivalent native plant species. Functionally equivalent species should have similar flower structure, similar flower color, and flower during the same part of the year as the exotic species they will be replacing. 
